                ...
                {DMList.length > 0 &&
                    DMList.map((DMList, index) => {
                        return (
                            ...
                            <div key={index} className={DMList.userNo === loginUserNo ? style.mySend : style.other}>
                                <span className={DMList.userNo === loginUserNo ? null :style.otherPhoto}>
                                    {DMRoom.sysName != null ? (<img src={`/images/${DMRoom.sysName}`} className={style.profileImg}></img>) 
                                    : ( <img className={style.profileImg} src={`/images/test.jpg`}></img>)}
                                </span>
                                <span className={DMList.userNo === loginUserNo ? style.myMsg : style.otherMsg}>
                                    {DMList.message}
                                </span>
                            </div>
                            {DMList.userNo === loginUserNo ? (
                                    <div className={style.mySendTime}>{DMList.formedWriteDate}</div>
                                ) : (
                                    <div className={style.otherTime}>{DMList.formedWriteDate}</div>
                                )}
                            </div>
                        );
                    })}
